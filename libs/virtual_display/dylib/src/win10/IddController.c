...
BOOL InstallUpdate(LPCWSTR fullInfPath, PBOOL rebootRequired)
{
    ...
    ))
    {
        ...
        if (error != 0)
        {
            ...
            {
            case 0x109:
                SetLastMsg("Failed InstallUpdate UpdateDriverForPlugAndPlayDevicesW, error: 0x%x, %s  Please try: Reinstall RealControl with the cert option.\n", error, errorString == NULL ? "(NULL)\n" : errorString);
                break;
            case 0xe0000247:
                SetLastMsg("Failed InstallUpdate UpdateDriverForPlugAndPlayDevicesW, error: 0x%x, %s  Please try: \n1. Check the device manager and event viewer.\n2. Uninstall \"RustDeskIddDriver Device\" in device manager, then reinstall RealControl with the cert option.\n", error, errorString == NULL ? "(NULL)\n" : errorString);
                break;
            default:
...
BOOL DeviceCreateWithLifetime(SW_DEVICE_LIFETIME *lifetime, PHSWDEVICE hSwDevice)
{
    ...
    PCWSTR description = L"RealControl Idd Driver";
