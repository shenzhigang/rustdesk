...
def make_parser():
    ...
    parser.add_argument(
        "--app-name", type=str, default="RealControl", help="The app name."
    )
    parser.add_argument(
...
def gen_auto_component(app_name, dist_dir):
    return gen_content_between_tags(
        "Package/Components/RealControl.wxs",
        "<!--$AutoComonentStart$-->",
        "<!--$AutoComponentEnd$-->",
...
def replace_app_name_in_langs(app_name):
    ...
    for file_path in langs_dir.glob("*.wxl"):
        with open(file_path, "r", encoding="utf-8") as f:
            lines = f.readlines()
        for i, line in enumerate(lines):
            lines[i] = line.replace("RealControl", app_name)
...
def update_license_file(app_name):
    if app_name == "RealControl":
        return
    license_file = Path(sys.argv[0]).parent.joinpath("Package/License.rtf")
    with open(license_file, "r", encoding="utf-8") as f:
        license_content = f.read()
    license_content = license_content.replace("website realcontrol.com and other ", "")
    license_content = license_content.replace("RealControl", app_name)
